...
Generates images using the Jimeng API and downloads them to the /pic folder.
"""
...
def load_env_file(env_file: str = None):
    """
    Load simple KEY=VALUE pairs from a local env file without adding dependencies.
...
def build_headers(session_id: str, jmapi_key: str = None, json_content: bool = False):
    headers = {
...
def generate_text_to_image(
    ...
    headers = build_headers(session_id, jmapi_key, json_content=True)
...
def generate_image_to_image(
    ...
    headers = build_headers(session_id, jmapi_key)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate images using Jimeng API",
    ...
    for subparser in [text_parser, image_parser]:
        subparser.add_argument(
            "--session-id",
            type=str,
            default=os.environ.get("JIMENG_SESSION_ID"),
            help="Jimeng session ID (with region prefix if needed: us-/hk-/jp-/sg-)"
        )
        subparser.add_argument(
            "--jmapi-key",
            type=str,
            default=os.environ.get("JMAPI_KEY"),
            help="Gateway key for remote deployments that require X-JMAPI-Key"
        )
        subparser.add_argument(
            "--model",
            type=str,
        ...
        subparser.add_argument(
            "--api-url",
            type=str,
            default=os.environ.get("JMAPI_BASE_URL", "https://jmapi.namooca.com"),
            help="Jimeng API base URL (default: JMAPI_BASE_URL or https://jmapi.namooca.com)"
        )
        subparser.add_argument(
            "--output-dir",
            type=str,
    ...
    if not args.session_id:
        print("❌ Error: session ID is required. Pass --session-id or set JIMENG_SESSION_ID in namoo-jmapi/.env.local")
        sys.exit(1)
